const SGO_OS_CHECKLIST = (() => {
  ...
  function salvarRespostas(sessionId, payloadOuOsId, respostasTalvez) {
    ...
    SGO_DATA.log("OS_CHECKLIST_RESPOSTAS_SALVAR", sessao.usuario, "Respostas salvas para OS=" + osId, "OS");
  ...
  function validarObrigatoriosInterno(osOuId) {
    ...
    if (!modelo) {
      pendencias.push("Nao existe questionario configurado para este tipo de OS/equipamento.");
      return { ok: false, pendencias: pendencias, modelo: null };
    }
    ...
    perguntas.forEach(function(p) {
      ...
      if ((snBool_(p.EXIGE_FOTO_SE_NAO_OK) && naoOk) || (snBool_(modelo.EXIGE_FOTO) && snBool_(p.OBRIGATORIO))) {
  ...
  function respostaNaoConforme_(valor) {
    ...
    return ["NAO", "N", "NAO_OK", "NAO_CONFORME", "NC", "REPROVADO"].indexOf(v) >= 0;
  }
