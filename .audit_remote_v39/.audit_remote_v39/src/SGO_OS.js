const SGO_OS = (() => {
  ...
  function atualizar(sessionId, id, payload) {
    ...
    if (atual.ASSINATURA_ID && payload && payload.ENCERRAMENTO_TECNICO && SGO_UTILS.safe(payload.ENCERRAMENTO_TECNICO) !== SGO_UTILS.safe(atual.ENCERRAMENTO_TECNICO)) {
      return { success: false, message: "Assinatura existente bloqueia edicao do encerramento tecnico." };
    }
  ...
  function salvarRegistroTecnico(sessionId, id, payload) {
    ...
    if (!os) return { success: false, message: "OS nao encontrada." };
    if (!podeExecutar_(sessao, os)) return { success: false, message: "Acesso negado." };
    if (os.ASSINATURA_ID && payload && payload.ENCERRAMENTO_TECNICO && SGO_UTILS.safe(payload.ENCERRAMENTO_TECNICO) !== SGO_UTILS.safe(os.ENCERRAMENTO_TECNICO)) {
      return { success: false, message: "Assinatura existente bloqueia edicao do encerramento tecnico." };
    }
    ...
    SGO_DATA.log("OS_SALVAR_REGISTRO_TECNICO", sessao.usuario, "Registro tecnico salvo: " + os.ID, "OS");
  ...
  function concluirTecnica(sessionId, id, payload) {
    ...
    return ok ? { success: true, message: "OS concluida tecnicamente. Aguardando aprovacao administrativa." } : { success: false, message: "Erro ao concluir OS." };
  }
  ...
  function validarConclusaoInterna_(os) {
    ...
    if (tipoExigeEvidencia_(os.TIPO_OS)) {
      const fotos = safeGetMany_(SHEET_FOTOS, "OS_ID", os.ID, DB_OS);
      if (!fotos.length) pendencias.push("Registre ao menos uma evidencia/foto da execucao.");
    }

    if (tipoExigeAssinatura_(os.TIPO_OS)) {
      const assinaturas = safeGetManyPrincipal_(SHEET_ASSINATURAS, "OS_ID", os.ID);
      if (!os.ASSINATURA_ID && !assinaturas.length) pendencias.push("Colete a assinatura do cliente/responsavel.");
    }
  ...
  function tipoDocStatus_(statusOs) {
    ...
    if (["APROVADA", "FATURADA"].indexOf(s) >= 0) return { label: "DOCUMENTO OFICIAL", cor: "#065f46", bg: "#d1fae5" };
    if (["CONCLUIDA_TECNICAMENTE", "EM_APROVACAO"].indexOf(s) >= 0) return { label: "RELATORIO TECNICO DE ATENDIMENTO", cor: "#1e40af", bg: "#dbeafe" };
    return { label: "PREVIA — NAO OFICIAL", cor: "#92400e", bg: "#fef3c7" };
  }
  ...
  function montarHtmlOS_(pacote, sessao) {
    ...
    const watermarkStyle = tipoDoc.label !== "DOCUMENTO OFICIAL"
    ...
    '<div style="background:' + tipoDoc.bg + ';color:' + tipoDoc.cor + ';padding:8px 16px;border-radius:6px;font-size:12px;font-weight:800;text-align:center;letter-spacing:.08em;text-transform:uppercase;margin-bottom:16px;">' + esc_(tipoDoc.label) + '</div>' +
      '<section class="df-cover">' +
        '<div>' +
          ...
          '<div class="df-subtitle">Relatorio executivo de atendimento, rastreabilidade, evidencias e aprovacao tecnica</div>' +
      ...
      dfSection_("Demanda", '<div class="df-summary" style="grid-template-columns:1fr 1fr;">' +
        noteCard_("Relato do cliente", os.RELATO_CLIENTE) +
        noteCard_("Missao tecnica", os.MISSAO_TECNICA) +
      '</div>') +

      dfSection_("Execucao tecnica", tabelaDoc_([
        ["Inicio", formatarDataDoc_(os.DATA_INICIO || missao.CHECKIN_EM), "Fim", formatarDataDoc_(os.DATA_CONCLUSAO || missao.CHECKOUT_EM)],
        ["Check-in", [formatarDataDoc_(missao.CHECKIN_EM), missao.CHECKIN_ENDERECO].filter(Boolean).join(" | "), "Check-out", [formatarDataDoc_(missao.CHECKOUT_EM), missao.CHECKOUT_ENDERECO].filter(Boolean).join(" | ")],
        ["Tecnico", tecnicoNome, "Status final", os.STATUS],
        ["Encerramento tecnico", os.ENCERRAMENTO_TECNICO, "Diagnostico final", os.DIAGNOSTICO_FINAL]
      ])) +

      dfSection_("Checklist", tabelaChecklistDoc_(pacote.checklist)) +
      dfSection_("Materiais e pecas", tabelaMateriaisDoc_(pacote.materiais, mostrarCustos)) +
      dfSection_("Evidencias", tabelaEvidenciasDoc_(pacote.fotos)) +
      dfSection_("Assinatura e aprovacao", '<div class="df-sign">' +
        tabelaDoc_([
          ["Assinado por", assinatura.ASSINADO_POR || os.ASSINADO_POR, "Cargo", assinatura.CARGO],
          ["Assinado em", formatarDataDoc_(assinatura.ASSINADO_EM || os.ASSINADO_EM), "Aprovado por", os.APROVADO_POR || os.APROVADOR_ID],
          ["Aprovado em", formatarDataDoc_(os.APROVADO_EM || os.DATA_APROVACAO), "Status", os.APROVACAO_ADMIN || os.STATUS]
        ]) +
