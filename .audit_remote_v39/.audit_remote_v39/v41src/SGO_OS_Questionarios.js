...
function checklistModelosListar(sessionId, filtros) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.listarModelos(sessionId, filtros))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloCriar(sessionId, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.criarModelo(sessionId, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloAtualizar(sessionId, id, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.atualizarModelo(sessionId, id, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloInativar(sessionId, id) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.inativarModelo(sessionId, id))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloClonar(sessionId, id) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.clonarModelo(sessionId, id))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloObterCompleto(sessionId, id) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.obterModeloCompleto(sessionId, id))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistModeloPorTipoOSTipoEquipamento(sessionId, tipoOs, tipoEquipamento) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.modeloPorTipoOSTipoEquipamento(sessionId, tipoOs, tipoEquipamento))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

/* --- 2. Perguntas ---------------------------------------------- */
function checklistPerguntaCriar(sessionId, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.criarPergunta(sessionId, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistPerguntaAtualizar(sessionId, id, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.atualizarPergunta(sessionId, id, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistPerguntaExcluir(sessionId, id) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.excluirPergunta(sessionId, id))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

/* --- 3. Respostas ---------------------------------------------- */
function checklistRespostasSalvar(sessionId, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.salvarRespostas(sessionId, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function checklistRespostasListarPorOS(sessionId, osId) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.listarRespostasPorOS(sessionId, osId))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

/* --- 4. Validação ---------------------------------------------- */
function checklistValidarObrigatorios(sessionId, osId) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.validarObrigatorios(sessionId, osId))); }
  catch(e) { return { success: false, message: "Erro: " + e.message, pendencias: [e.message] }; }
}
...
function checklistSeedQuestionariosIniciais(sessionId) {
  try {
    ...
    var existentes = SGO_OS_CHECKLIST.listarModelos(sessionId, {});
    var nomesExistentes = (existentes.items || []).map(function(m) {
      return SGO_UTILS.safeUpper(SGO_UTILS.safe(m.NOME));
    });
    ...
    if (nomesExistentes.indexOf("ATENDIMENTO CORRETIVO GERAL") < 0) {
      var modeloA = SGO_OS_CHECKLIST.criarModelo(sessionId, {
    ...
    if (nomesExistentes.indexOf("MANUTENCAO PREVENTIVA AUTOCLAVE") < 0) {
      var modeloB = SGO_OS_CHECKLIST.criarModelo(sessionId, {
    ...
    log.forEach(function(l) { Logger.log("[SEED] " + l); });
...
function osChecklistListarTemplates(sessionId, tipoOs) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.listarTemplates(sessionId, tipoOs))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}

function osChecklistSalvarTemplate(sessionId, payload) {
  try { return JSON.parse(JSON.stringify(SGO_OS_CHECKLIST.salvarTemplate(sessionId, payload))); }
  catch(e) { return { success: false, message: "Erro: " + e.message }; }
}
...
function testeSeedQuestionariosOS() {
  var USUARIO = "COLOCAR_USUARIO_AQUI";
  var SENHA   = "COLOCAR_SENHA_AQUI";

  Logger.log("=== testeSeedQuestionariosOS ===");

  var auth = login(USUARIO, SENHA);
  ...
  var resultado = checklistSeedQuestionariosIniciais(auth.sessionId);
