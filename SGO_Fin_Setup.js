...
// FLASH.6.8 — Recarga controlada pelo financeiro. Helpers privados + 4 funções públicas.

function _f68ChaveIdempotencia_(cartaoId, valor) {
  var d = new Date();
  var ds = d.getFullYear() + ('0'+(d.getMonth()+1)).slice(-2) + ('0'+d.getDate()).slice(-2);
  var v  = (Math.round((valor || 0) * 100) / 100).toFixed(2);
  return 'RECARGA_BRUNA_5553116198_' + String(cartaoId).trim() + '_' + v + '_' + ds;
}
...
function _f68BuscarCartaoAtivo_(ss, cpf, cartaoId) {
  ...
  for (var i = 1; i < dados.length; i++) {
    ...
    return {
      encontrado: true, cpf: rowCpf, cartaoId: cid, status: st,
...
function _f68ValidarPayload_(payload, ss, cpfPiloto) {
  ...
  var normalizado = bloqueios.length === 0 ? {
    cpf: cpf, cartaoId: cartaoId, valor: valor,
    finalidade: finalidade, observacao: String(payload.observacao || '').trim(),
...
function PREVER_RECARGA_FLASH_CONTROLADA_FINANCEIRO_SEM_GRAVAR(payload) {
  ...
  try {
    ...
    var v = _f68ValidarPayload_(payload, amb.ss, CPF_PILOTO);
...
function EXECUTAR_RECARGA_FLASH_CONTROLADA_FINANCEIRO(payload) {
  ...
  var resultado = {
    ...
    cpf: CPF_PILOTO, cartaoId: null, valor: null,
    gravacaoReal: false,
...
function FINALIZAR_FLASH68_RECARGA_FINANCEIRO_PRONTA_SEM_EXECUTAR() {
  var CPF_PILOTO = '5553116198';
  var CARTAO_PILOTO = 'REAL-65BC435CB7E1';
  var resultado = {
    success: false, ok: false, fase: 'FLASH.6.8.FECHAMENTO',
    ambiente: null, cpf: CPF_PILOTO, cartaoId: CARTAO_PILOTO,
    backendPreviaExiste: false, backendExecucaoExiste: false,
    backendAuditoriaExiste: false, uiPreparada: true,
    brunaEncontrada: false, cartaoEncontrado: false,
    liberacaoGeralFalse: false, operacaoControladaAtiva: false,
    brunaAutorizada: false,
    prontoParaFinanceiroExecutar: false,
    gravacaoReal: false, bloqueios: [], avisos: []
  };
  try {
    var amb = _f410ValidarAmbientePV2_();
    ...
    resultado.backendAuditoriaExiste  = typeof AUDITAR_FLASH68_POS_RECARGA_CONTROLADA_SEM_GRAVAR === 'function';
    if (!resultado.backendPreviaExiste)   resultado.bloqueios.push('PREVER_RECARGA_FLASH_CONTROLADA_FINANCEIRO_SEM_GRAVAR nao encontrada.');
    if (!resultado.backendExecucaoExiste) resultado.bloqueios.push('EXECUTAR_RECARGA_FLASH_CONTROLADA_FINANCEIRO nao encontrada.');
    if (!resultado.backendAuditoriaExiste) resultado.bloqueios.push('AUDITAR_FLASH68_POS_RECARGA_CONTROLADA_SEM_GRAVAR nao encontrada.');
    var props = PropertiesService.getScriptProperties();
    resultado.liberacaoGeralFalse     = props.getProperty('FLASH_LIBERACAO_GERAL') !== 'true';
    resultado.operacaoControladaAtiva = props.getProperty('FLASH_OPERACAO_CONTROLADA_ATIVA') === 'true';
    resultado.brunaAutorizada         = !!_f413CPFAutorizado_(CPF_PILOTO);
    if (!resultado.liberacaoGeralFalse)     resultado.bloqueios.push('REGRESSAO: liberacao geral esta true.');
    if (!resultado.operacaoControladaAtiva) resultado.bloqueios.push('Operacao controlada nao ativa.');
    if (!resultado.brunaAutorizada)         resultado.bloqueios.push('CPF da Bruna nao autorizado no guard FLASH.4.13.');
    var cartao = _f68BuscarCartaoAtivo_(amb.ss, CPF_PILOTO, CARTAO_PILOTO);
    resultado.brunaEncontrada  = cartao.encontrado;
    resultado.cartaoEncontrado = cartao.encontrado;
    if (!cartao.encontrado) {
      resultado.bloqueios.push('Cartao ' + CARTAO_PILOTO + ' nao encontrado em FIN_CARTOES para CPF ' + CPF_PILOTO + '.');
    }
    resultado.prontoParaFinanceiroExecutar =
      resultado.backendPreviaExiste && resultado.backendExecucaoExiste &&
      resultado.brunaAutorizada && resultado.cartaoEncontrado &&
      resultado.liberacaoGeralFalse && resultado.operacaoControladaAtiva;
    resultado.success = resultado.bloqueios.length === 0;
    resultado.ok = resultado.prontoParaFinanceiroExecutar;
    if (resultado.ok) {
      resultado.avisos.push('FLASH.6.8 pronto. Financeiro pode executar PREVER_RECARGA_FLASH_CONTROLADA_FINANCEIRO_SEM_GRAVAR(payload) para dry-run, depois EXECUTAR_RECARGA_FLASH_CONTROLADA_FINANCEIRO(payload) para recarga real.');
      resultado.avisos.push('Payload esperado: {cpf:"5553116198",cartaoId:"REAL-65BC435CB7E1",valor:<N>,finalidade:"<texto>",observacao:"<texto>",confirmarFinanceiro:true}');
    }
  } catch (e) {
    resultado.bloqueios.push('Erro FLASH.6.8.FECHAMENTO: ' + (e.message || String(e)));
  }
  Logger.log(JSON.stringify(resultado, null, 2));
  return resultado;
}
